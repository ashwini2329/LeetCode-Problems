...
for (i = 0; i < stringArr.length; i++) {
  let shortestLength = BigInt("12673094");
  for (j = 0; j < stringArr.length; j++) {
    if (stringArr[j] === c && Math.abs(i - j) < shortestLength) {
      shortestLength = Math.abs(i - j);
    }
  }
  resultArr.push(shortestLength);
}

console.log(`result Array = ${resultArr}`); // return resultArr;
