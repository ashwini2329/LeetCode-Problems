...
You are given a 0-indexed string s consisting of only lowercase English letters, where each letter 
in s appears exactly twice. You are also given a 0-indexed integer array distance of length 26.
...
public class checkDistance {
    public static void main(String[] args) {
        ...
        for (int i=0 ; i < 26 ; ++i) {
            alphabets.put(((char)(97+i)), distance[i]);
        }
            ...
            if(!visited.contains(s.charAt(i))) {
                for(int j=i+1; j<stringLength; j++) {
                    if(Character.compare(s.charAt(i), s.charAt(j)) == 0) {
                        int noOfLetters = (j-i) - 1;
                        int difference = alphabets.get(s.charAt(i));
                        if(difference != noOfLetters) {
                            result = false;
                            break;
                        }
                    }
