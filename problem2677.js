...
Output: [[1,9,6],[3,2]]
Explanation: The arr has been split into subarrays with 3 elements. However, only two 
elements are left for the 2nd subarray.
Example 3:
...
for (let i = 0; i < arr.length; i++) {
  counterArr.push(arr[i]);
  counter++;
  if (i === arr.length - 1 || counter === size) {
    resultArr.push(counterArr);
  }
  if (counter === size) {
    counter = 0;
    counterArr = [];
