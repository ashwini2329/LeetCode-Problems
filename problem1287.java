/**
 
Given an integer array sorted in non-decreasing order, there is exactly one integer in the array 
that occurs more than 25% of the time, return that integer.

 

Example 1:

Input: arr = [1,2,2,6,6,6,6,7,10]
Output: 6
Example 2:

Input: arr = [1,1]
Output: 1
 

Constraints:

1 <= arr.length <= 104
0 <= arr[i] <= 105

 */


/**********************************************************************
        1287. Element Appearing More Than 25% In Sorted Arrays
**********************************************************************/

public class problem1287 {
    public static void main(String[] args) {
        int[] arr = {1,2,2,6,6,6,6,7,10};   // Sample Input
        
        int quarterPercentage = arr.length/4;
        int result = -1;
        
        for(int i=0; i<arr.length; i++) {
            int count = 0;
            for(int j=i; j<arr.length; j++) {
                if(arr[i] == arr[j]) {
                    count++;
                }
            }
            if(count > quarterPercentage) {
                result = arr[i];
            }
        }

        System.out.println("result = " + result);   // return result;
    }
}
