...
public class wordSummation {
    public static void main(String[] args) {
        String firstWord = "acb", secondWord = "cba", targetWord = "cdb";   // Sample Input
        ...
        for(int i=1; i<=firstWordLength; i++) {
            int numericValue = (int)firstWord.charAt(i-1) - 97;
            firstWordString += String.valueOf(numericValue);
        }

        for(int i=1; i<=secondWordLength; i++) {
            int numericValue2 = (int)secondWord.charAt(i-1) - 97;
            secondWordString += String.valueOf(numericValue2);
        }

        for(int i=1; i<=targetWordLength; i++) {
            int numericValue3 = (int)targetWord.charAt(i-1) - 97;
            targetWordString += String.valueOf(numericValue3);
        }

        result = (Integer.parseInt(firstWordString) + 
                Integer.parseInt(secondWordString)) == 
                Integer.parseInt(targetWordString);
