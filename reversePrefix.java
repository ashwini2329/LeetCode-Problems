...
public class reversePrefix {
    public static void main(String[] args) {
        ...
        if(index >= 0) {
            ...
            StringBuilder reverse = new StringBuilder(subs);
            String reversedSubstring = reverse.reverse().toString();

            result += reversedSubstring;
            result += word.substring(index + 1, word.length());
            System.out.println("result = " + result );      // return result;
        } else {
            System.out.println("string did not change = " + word);      // return word
        }
    }
}
