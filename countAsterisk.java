...
public class countAsterisk {
    public static void main(String[] args) {
        String s = "yo|uar|e**|b|e***au|tifu|l";    // Sample Input
        ...
        for(int i=0; i<s.length(); i++) {
           ...
           if((barCount == 0 && (Character.compare(s.charAt(i), '*') == 0) ) || 
           ((barCount == 2) && (Character.compare(s.charAt(i), '*') == 0))) {
                starCount++;
           }
           if((barCount == 3) && (Character.compare('|', s.charAt(i))==0)) {
            barCount = 1;
           }
        }

        System.out.println("starCount at  = " + starCount );    // return starCount;
    }
}
