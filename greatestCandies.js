...
let greatestAmongCandies = 0;
for (i = 0; i < candies.length; i++) {
  if (candies[i] >= greatestAmongCandies) {
    greatestAmongCandies = candies[i];
  }
}
console.log(`Greatest among candies = ${greatestAmongCandies}`);

let newCandies = candies;
for (i = 0; i < candies.length; i++) {
  newCandies[i] += extraCandies;
}
console.log(`New Candies Array after adding extraCandies = ${newCandies}`);

let result = [];
for (i = 0; i < newCandies.length; i++) {
  if (newCandies[i] >= greatestAmongCandies) {
    result.push(true);
  } else {
    result.push(false);
  }
